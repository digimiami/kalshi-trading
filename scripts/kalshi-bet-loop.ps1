...
# === 1. Check balance ===
try {
    $BalanceOutput = & "{kalshi-cli-path}" --prod portfolio balance --json 2>&1 | ConvertFrom-Json
...
$RestingOrders = try {
    & "{kalshi-cli-path}" --prod orders list --status resting --json 2>&1 | ConvertFrom-Json
} catch { $null }
...
$Positions = try {
    & "{kalshi-cli-path}" --prod portfolio positions --json 2>&1 | ConvertFrom-Json
} catch { $null }
